...
4. **数据备份** - 在管理面板点击"导出数据"备份番号，点击"导入数据"恢复备份
...
### v1.5.4 (2026-02-12)
- 🎯 新增前缀去重功能：自动检测并删除作为其他番号前缀的不完整番号
- 📊 清理结果优化：分类显示重复/无效、前缀去重、跨列表重复的清理数量
- 🔧 清理详情展示：显示被前缀去重删除的具体番号列表

### v1.5.3 (2026-02-08)
- 🎯 新增手动去重功能：管理窗口添加"🧹 去重并标准化数据"按钮
- 🔄 智能数据清理：一键去除重复番号，自动标准化所有数据（转大写、去空格）
- 🛡️ 跨列表重复处理：自动清理同时存在于已看/想看列表中的番号
- 🎨 清理结果可视化：直观显示清理前后的数量变化
