...
// @version      1.1.2
// @description  [核心] 抓取豆瓣看过/想看的电影与剧集，自动匹配IMDb编号，一键导出Trakt可导入JSON；[辅助] 断点续传、暂停续跑、IMDb缓存、安全验证自动暂停、未匹配清单；[资源] 玻璃拟态悬浮面板与实时日志。
// @author       RiTian96
// @match        *://movie.douban.com/*
// @icon         https://movie.douban.com/favicon.ico
// @icon         https://www.google.com/s2/favicons?sz=64&domain=movie.douban.com
// @grant        GM_addStyle
// @grant        GM_setValue
// @grant        GM_getValue
...
(function () {
    ...
    function injectStyles() {
        ...
        GM_addStyle(`
            #db-trakt-fab {
                ...
                box-shadow: 0 18px 45px rgba(0,0,0,0.35), 0 0 0 1px rgba(255,255,255,0.05) inset;
            ...
            #db-trakt-fab:hover { transform: translateY(-2px) scale(1.05); filter: brightness(1.15); }
            #db-trakt-fab:active { transform: translateY(0) scale(0.96); }
            ...
            .db-trakt-panel {
                ...
                border-radius: 20px;
                box-shadow: 0 30px 90px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.05) inset;
                backdrop-filter: blur(24px) saturate(180%);
                -webkit-backdrop-filter: blur(24px) saturate(180%);
                color: #f5f5f7;
                ...
                overflow: hidden;
                animation: dbTraktPop 0.3s cubic-bezier(0.16, 1, 0.3, 1);
            }
            @keyframes dbTraktPop {
                from { opacity: 0; transform: translateY(12px) scale(0.97); }
                to { opacity: 1; transform: translateY(0) scale(1); }
            }
            .db-trakt-panel.hidden { display: none; }
            ...
            .db-trakt-title { font-size: 14px; font-weight: 700; letter-spacing: 0.4px; }
            .db-trakt-header-btns button {
            ...
            .db-trakt-header-btns button:hover { color: #fff; }

            .db-trakt-body { padding: 14px 16px 16px 16px; overflow-y: auto; }
            .db-trakt-body::-webkit-scrollbar { width: 5px; }
            .db-trakt-body::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.15); border-radius: 3px; }

            .db-trakt-label { font-size: 12px; color: rgba(255,255,255,0.55); margin-bottom: 8px; letter-spacing: 0.3px; }
            .db-trakt-checks { display: grid; grid-template-columns: 1fr 1fr; gap: 6px 10px; margin-bottom: 14px; }
            .db-trakt-checks label {
                display: flex; align-items: center; gap: 6px; cursor: pointer;
                font-size: 12px; color: rgba(255,255,255,0.9);
            }
            .db-trakt-checks input { accent-color: #41bd55; cursor: pointer; }

            .db-trakt-row { display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-bottom: 12px; }
            .db-trakt-row .db-trakt-label { margin-bottom: 0; }
            ...
            .db-trakt-panel select:focus, .db-trakt-panel input[type="text"]:focus {
                border-color: rgba(65, 189, 85, 0.55);
                box-shadow: 0 0 0 3px rgba(65, 189, 85, 0.12);
            }
            .db-trakt-panel select option { background: #1e1e22; color: #f5f5f7; }

            .db-trakt-btns { display: flex; gap: 8px; margin-bottom: 12px; }
            .db-trakt-btn {
                flex: 1;
                padding: 9px 6px;
                border: none;
            ...
            .db-trakt-btn:hover { transform: translateY(-2px); filter: brightness(1.15); }
            .db-trakt-btn:active { transform: translateY(0) scale(0.97); }
            .db-trakt-btn:disabled { opacity: 0.4; cursor: not-allowed; transform: none; filter: none; }
            .db-trakt-btn-primary {
                background: linear-gradient(135deg, #41bd55, #2e9e46);
                box-shadow: 0 4px 15px rgba(65, 189, 85, 0.3);
                border-color: transparent;
            }
            .db-trakt-btn-danger { background: linear-gradient(135deg, #e74c3c, #c0392b); border-color: transparent; }

            .db-trakt-progress { margin-bottom: 10px; }
            .db-trakt-progress-text { font-size: 12px; color: rgba(255,255,255,0.85); margin-bottom: 6px; }
            ...
            .db-trakt-stats { font-size: 12px; color: rgba(255,255,255,0.6); margin-bottom: 10px; }
            ...
            .db-trakt-link-btn:hover { background: rgba(255,255,255,0.1); color: #fff; }
            ...
            .db-trakt-log a { color: #7ed957; }
            .db-trakt-log .warn { color: #f39c12; }
            .db-trakt-log .error { color: #e74c3c; }
            .db-trakt-log .success { color: #41bd55; }

            .db-trakt-tip { font-size: 11px; color: rgba(255,255,255,0.4); margin-top: 8px; line-height: 1.6; }
        `);
    }
    ...
    function showToast(content, options = {}) {
        ...
        div.style.cssText = `
            ...
            box-shadow: 0 8px 30px rgba(0,0,0,0.35); z-index: 2147483647;
    ...
    function updateStats() {
        ...
        const cacheEl = document.getElementById('db-trakt-cache');
        if (cacheEl) cacheEl.textContent = `IMDb 缓存 ${Object.keys(State.imdb).length} 条`;
    }
    ...
    function createUI() {
        if (document.getElementById('db-trakt-fab')) return;

        const fab = document.createElement('div');
        fab.id = 'db-trakt-fab';
        fab.title = '豆瓣 → Trakt 导出';
        fab.textContent = '🎬';
        document.body.appendChild(fab);

        const panel = document.createElement('div');
        panel.id = 'db-trakt-panel';
        panel.className = 'db-trakt-panel hidden';
        panel.innerHTML = `
            <div class="db-trakt-header">
                <span class="db-trakt-title">🎬 豆瓣 → Trakt 导出</span>
                <div class="db-trakt-header-btns">
                    <button id="db-trakt-min" title="收起">－</button>
                    <button id="db-trakt-close" title="关闭">×</button>
                </div>
            </div>
            <div class="db-trakt-body">
                <div class="db-trakt-label">导出范围</div>
                <div class="db-trakt-checks">
                    ${RANGES.map(r => `<label><input type="checkbox" data-range="${r.key}" checked> ${r.label}</label>`).join('')}
                </div>
                ...
                <div class="db-trakt-row">
                    <span class="db-trakt-label">豆瓣ID</span>
                    <input id="db-trakt-uid" type="text" placeholder="个人主页链接中的 ID">
                </div>

                <div class="db-trakt-btns">
                    <button id="db-trakt-start" class="db-trakt-btn db-trakt-btn-primary">▶ 开始</button>
                    <button id="db-trakt-pause" class="db-trakt-btn">⏸ 暂停</button>
                    <button id="db-trakt-resume" class="db-trakt-btn" style="display:none">⏵ 继续</button>
                ...
                <div class="db-trakt-row">
                    <span id="db-trakt-cache" class="db-trakt-label">IMDb 缓存 0 条</span>
                    <div style="display:flex; gap:6px;">
                        <button id="db-trakt-export" class="db-trakt-link-btn">导出JSON</button>
                        <button id="db-trakt-export-miss" class="db-trakt-link-btn">未匹配</button>
                        <button id="db-trakt-clear" class="db-trakt-link-btn">清缓存</button>
                    </div>
                </div>

                <div class="db-trakt-log" id="db-trakt-log"></div>
                <div class="db-trakt-tip">导入：trakt.tv/apps/import 选择 JSON 上传。观看日期取豆瓣标记日期 + 12:00Z，评分按 5 星制 ×2 换算。</div>
            </div>
        `;
        ...
        document.getElementById('db-trakt-min').addEventListener('click', () => panel.classList.add('hidden'));
        document.getElementById('db-trakt-close').addEventListener('click', () => {
            panel.remove();
            fab.remove();
        });
        ...
        panel.querySelectorAll('input[data-range]').forEach(b => {
            b.addEventListener('change', saveOptions);
        });
