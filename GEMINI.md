...
| **JavDB影片管理器** | 2.0.0 | `https://javdb.com/*` | 看过/想看影片屏蔽，智能评分分级（低分/高分）；搜索精确匹配高亮，详情页同步(含删除)，数据备份；支持欧美区番号 |
...
#### 🛡️ 内容管理类
- **JavDB影片管理器**: 看过/想看影片屏蔽，智能评分分级（低分/高分）；批量导入，大图预览，数据备份；支持欧美区番号
...
└── tampermonkey-scripts/       # 油猴脚本目录
    ...
    ├── javdb-manager.user.js   # JavDB管理器脚本 (v1.9.0)
