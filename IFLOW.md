...
| **JavDB影片管理器** | 1.5.4 | `https://javdb.com/*` | 已看/想看影片屏蔽，评分高亮，批量管理，欧美区番号支持，番号标准化存储，手动数据去重，前缀去重 |
...
#### 🛡️ 内容管理类
- **JavDB影片管理器**: 智能屏蔽已看/想看影片，评分高亮过滤，批量导入管理，欧美区番号支持，番号标准化存储防重复，手动数据去重
...
└── tampermonkey-scripts/       # 油猴脚本目录
    ...
    ├── javdb-manager.user.js   # JavDB管理器脚本 (v1.5.4)
